...
Wrk[$v$]. At the pair $(u,r_u(v))$ in Wrk[$v$] note that the last element of
Wrk[$u$] is $(v,r_v(u))$. Look up and assign references for the edge $uv$ in
Adj'. Remove $(v,r_v(u))$ from the back of Wrk[$u$].
\end{algorithm}
...
Let $G$ be a plane graph. We may add edges to $G$ until it is triangulated
and path $2$-color the outer triangle. Applying Poh's algorithm computes a path
$3$-coloring of $G$.
