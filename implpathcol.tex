...
the sequence of graphs $\{G_k\}_{k\in\mathbb{N}}$ on which it displays
...
better than it did on random graphs. The structured nature of each
graph $G_k$ results in a memory layout where vertices close to one another in the graph
have their adjacency lists stored close together in memory. Thus the memory
access pattern of Algorithm~\ref{A:poh_linear} run on $G_k$ is almost linear.
...
Algorithm~\ref{A:poh_linear} is run on a graph
$G_k$ from Figure~\ref{F:poh_bfs} there will never be more
than one recursive frame on the stack, and thus there is
