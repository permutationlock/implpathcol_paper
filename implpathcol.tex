...
We discuss implementations of two algorithms that construct
path colorings of planar graphs.

The first algorithm, based on a proof
by Poh~\cite{Poh1990},
is given a planar graph;
it produces a path coloring of the given graph
using three colors.

The second algorithm performs a list-coloring generalization
of the above.
It is based on two similar proofs:
one by Hartman~\cite{Har1997},
the other by \v{S}krekovski~\cite{Skr1999}.
...
Links to implementations of the two algorithms are provided.
\end{abstract}
...
a \defterm{linear forest}, that is, a forest of maximum degree at most $2$,
or, equivalently, a disjoint union of paths.
...
based on proofs of the above results.
...
along with a similar proof of a weaker result by \v{S}krekovski.
The algorithm is given a planar graph
