...
A \defterm{(simple) graph} $G=(V,E)$ is a set $V(G)$ of vertices
and a set $E(G)$ of unordered pairs of vertices. We write $n(G)=|V(G)|$
...
\defterm{incident} to $uv$. The set of neighbors of $v$ is written
$N(v)$. The degree of $v$ is $\text{deg}(v)=|N(v)|$.
We write $v\in G$ and $uv\in G$ when it is clear $v$ is a vertex and $uv$ an edge.
