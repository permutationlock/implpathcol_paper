...
We often simply write $v\in G$ or $uv\in G$ when it is clear that
$v$ is a vertex or $uv$ an edge.
...
is a coloring such that each color class induces a union of disjoint
paths. A graph $G$ is \defterm{path $k$-colorable} if $G$
...
A plane graph $G$ is \defterm{triangulated} if every face is a
\defterm{triangle}: a length $3$ cycle. If every face but the
outer face is a triangle, then $G$ is
\defterm{weakly triangulated}.
We say $G$ is \defterm{connected} if given any $u,v\in G$, there exists a
$u,v$-path in $G$,
and \defterm{biconnected} if $G-v$ is connected
for every $v\in G$.
If $G$ is biconnected and $n(G)\ge 3$, then the outer face is a cycle.
...
The order of each
adjacency list represents the embedding.
For $v\in G$ and $u\in N(v)$, we define
$\text{next}_v(u)$ to be
...
adjacency list representation may be constructed in $\mathcal{O}(m)$ time.
...
$(v_1, v_1v_3, 1)$ form a valid input for Algorithm~\ref{A:poh_linear}.
...
the blocks (maximal biconnected subgraphs) of $G-zu_1$.
...
Let $N$ be an array such that for each $v\in C$, $N[v]=(r_1,r_2)$
contains
...
$C[x,u_1]$ and $C[u_2,y]$ into $C_1[z_1,y_1]$ in $G_1$ without needing
to re-mark each vertex in $C[x,u_1]$ with $S[y]$, see Figure~8.
\end{algorithm}
