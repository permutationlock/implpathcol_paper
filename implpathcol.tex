...
If $H$ is a subgraph of $G$, let $N(H) = \bigcup_{v\in V(H)} N(v)\setminus V(H)$ be the set of vertices in $G-H$ with a
neighbor in $H$.
...
$C$. Let $C_1=P_1+P_3+uu_1+ww_1$ and
$C_2=P_2+P_3+uu_2+ww_2$. The subgraphs
...
$C_1=C[v_2,v_1]+v_1v_2$
and $C_2=C[v_1,v_2]+v_1v_2$.
...
For each vertex $v\in P_3$, assign $c[v]\leftarrow c(P_3)$, where $c(P_3)\in
\{1,2,3\}-\{c[u_1],c[u_2]\}$.
...
$\text{Int}(C_1)$, where $C_1=C[u_1,v_1]+P_3+v_1u+v_1w$;}
\item{with input $(u, w, u_2, v_2, uu_2)$ to color
$\text{Int}(C_2)$, where $C_2=C[v_2,u_2]+P_3+uu_2+wv_2$;}
\item{with input $(v_1, w_1, v_2, w_2, v_1v_2)$
to color $\text{Int}(C_3)$, where $C_3=C[v_1,v_2]+v_1v_2$.}
\end{enumerate}
...
$v\in\text{Int}(C)-C$.
Let $S$ be a length $n$ array of integer marks and let $m(P_1)$ be an integer
such that for each $v\in\text{Int}(C)-C$ we have $S[v]=m(P_1)$ if and only if
$v\in N(P_1)$.

The concrete input will be the arrays $\text{Adj}$, $c$ and $S$,
along with the ordered pair $(u_1, u_1u_2, m(P_1))$ where $u_1u_2$ is
...
Let $m(P_3)$ be a new unique mark. We will
...
in $N(P_3)-V(C)$ with $m(P_3)$, and recursively
...
Assign $S[v]\leftarrow m(P_3)$ and $v\leftarrow\text{next}_{v_j}(v)$.
Go to Step 3.

\textbf{Case 3.2.} Suppose $c[v]=c[u_2]$, i.e. $v\in P_2$.
Make a recursive call with input $(v_i, v_is, m(P_3))$ to color
$\text{Int}(P_3[v_i,v_j]+C[v,s]+v_is+v_jv)$.
Assign $i\leftarrow j$, $v_js\leftarrow v_jv$, and
$v\leftarrow\text{next}_{v_j}(v)$. Go to Step 3.

\textbf{Case 3.3.} Suppose that $c[v]=0$ and $S[v]=m(P_1)$.
If $y=\text{NULL}$, assign $y\leftarrow v$.
Assign $S[v]\leftarrow m(P_3)$ and
$v\leftarrow\text{next}_{v_j}(v)$. Go to Step 3.
...
$S[v]\leftarrow m(P_3)$ and $v\leftarrow\text{next}_{v_j}(v)$.
Go to Step 4.

\textbf{Case 4.2.} Suppose that $c[v]\ne 0$. Make a recursive
call with input $(v_j, v_jt, m(P_3))$.

If $c[v]=c[u_1]$, i.e. $v\in P_1$, then the recursive
call colors $\text{Int}(C[v,t]+v_j+v_jt+v_jv)$.
Assign $v_jt\leftarrow v_jv$. Go to Step 4.

Otherwise,
$v=v_{j-1}$ and the recursive call
colors $\text{Int}(C[p_1(v),t]+P_3[v,v_j]+vp_1(v)+v_jt)$.
...
\[
    D = C[u_2,p_1(v_{j-1})]+P_3[v_1,v_{j-1}]+u_2v_1+
        v_{j-1} p_1(v_{j-1})
\]
