...
Let $G$ be a plane graph with $n$ edges and $m$ vertices.
All subgraphs of $G$ will be considered
...
if $G$ is triangulated, then $m=3n-6$.

If $v\in G$ and $u\in N(v)$, we define $\text{next}_v(u)$ to be
the neighbor of $v$ immediately counter-clockwise from $u$ around $v$,
and $\text{prev}_v(u)$ to be the neighbor immediately clockwise.
...
If $H,K$ are subgraphs of $G$, then
$H\cup K=(V(H)\cup V(K),E(H)\cup E(K))$ is their union subgraph in $G$.
Finally, define
$N(H) = \bigcup_{v\in V(H)} N(v)\setminus V(H)$ be the set of
