...
\begin{enumerate}
\itemsep0em
\item{one with input $(u_1, v_1, u, w, u_1u)$ to color
$\text{Int}(C[u_1,v_1]+P_3+v_1u+v_1w)$;}
\item{one with input $(u, w, u_2, v_2, uu_2)$ to color
$\text{Int}(C[v_2,u_2]+P_3+uu_2+wv_2)$;}
\item{one with input $(v_1, w_1, v_2, w_2, v_1v_2)$
to color $\text{Int}(C[v_1,v_2]+v_1v_2)$.}
\end{enumerate}
...
%   \node (v1) [label=above left:$v_1$] at (-2cm, 0.5cm) {};
%   \node (vi) [label=above right:$v_i$] at (0.5cm, 0.5cm) {};
%   \node (vi1) [label=below right:$v_{i+1}$] at (0.5cm, -0.75cm) {};
%   \node (vk) [label=below left:$v_k$] at (-2cm, -0.75cm) {};
...
%   \node (pv1) [label=above left:$v_1$] at (-2cm, 3.25cm) {};
%   \node (pvi) [label=above right:$v_i$] at (0.5cm, 3.25cm) {};
%   \node (pvi1) [label=below right:$v_{i+1}$] at (0.5cm, 2.0cm) {};
%   \node (pvk) [label=below left:$v_k$] at (-2cm, 2.0cm) {};
...
neighbor in $H$. 
For $P_1=C[u_1,w_1]$ define a funtion $p_1:N(P_1)\to V(P_1)$
...
\textbf{Base case.} Assign $u\leftarrow\text{next}_{u_1}(u_2)$.
If $c[u]\ne 0$, then either
$u=w_2$ or $u=w_1$.
But $C$ is an induced cycle in $G$, so $C$ must be a face.
Thus $\text{Int}(C)$ is already colored.

\textbf{Recursive step.} Assign $u\leftarrow\text{next}_{u_1}(u_2)$.
...
We claim that each $u,w$-path $P_3=v_1,v_2,\ldots,v_j$ constructed in 
Algorithm~\ref{A:poh_linear} is an induced path in $G$.
Suppose that $v_kv_\ell\in G$ with $0<k<\ell-1$. Observe that
\[
    D = C[v_0,p_1(v_{\ell-1})]+P_3[v_1,v_{\ell-1}]+v_0v_1+
        v_{\ell-1} p_1(v_{\ell-1})
\]
is a cycle in $G$ with $v_{k-1},v_k,v_{k+1}\in D$ and $v_{\ell}\not\in\text{Int}(D)$.
Therefore $v_kv_\ell\not\in\text{Int}(D)$ and
$v_kv_\ell$ lies between $v_{k-1}$ and $v_{k+1}$ counter-clockwise
