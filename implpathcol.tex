...
in the plane. All subgraphs of plane graphs will be considered
as plane graphs with the inherited embedding.
...
Let $G$ be a plane
graph. If $v\in G$ and $u\in N(v)$, we define $\text{next}_v(u)$ to be
...
Let $C$ be a cycle in $G$. If $u,v\in C$, we define
...
$G$ with outer face $C$.
If $P$ is a path in $G$, with $u,v\in P$, then
...
\begin{tikzpicture}[scale=0.75]
  ...
  \node (C) [scale=1.25] [draw=none, fill=none] at (-0.6cm, -0.1cm) {$C'$};
  \draw (v1) edge (vk);
  \draw (v1) edge (u);
  \draw (vk) edge (u);
  \draw (vi) edge (vi1);
  ...
  \node (PC) [scale=1.25] [draw=none, fill=none] at (-0.6cm, 2.65cm) {$C'$};
  \draw (pv1) edge (pvk);
  \draw (pv1) edge (pu);
  \draw (pvk) edge (pu);
  \draw (pvi) edge (pvi1);
...
If $H$ is a subgraph of $G$, let $N(H) = \bigcup_{v\in V(H)} N(v)$ be the set of vertices in $G$ with a
neighbor in $H$. 
For $P_1=C[u_1,w_1]$ define the funtion $p_1:N(P_1)\to V(P_1)$
sending each $v\in N(P_1)$ to its furthest neighbor along $P_1$, i.e.
$V(P_1[p_1(v),w_1])\cap N(v)=\{p_1(v)\}$.
...
assigned to $c[v]$ such that $c$ represents a path $3$-coloring of
$\text{Int}(C)$ extending the original $2$-coloring of $C$. Moreover, if
$v\in N(P_1)-C$, then $c[v]\ne c[u_1]$, and if $v\in N(P_2)-C$, then
$c[v]\ne c[u_2]$.
...
in $N(P_3)-C-P_3$ with $m(P_3)$, and recursively
...
\textbf{Case 3.4.} Suppose that $c[v]=c[u_1]$, i.e. $v\in P_1$.
...
\begin{tikzpicture}[scale=1.6]
    ...
    \node (vi) [label=above:${w_1}$] at (2.15cm, 1.0cm) {};
    \node (vi1) [label=below:${w_2}$] at (2.15cm, 0.0cm) {};
    \node (vs) [label=below:${s}$] at (0.5cm, -0.25cm) {};
    \node (vt) [label=below:${v}$] at (1.5cm, -0.25cm) {};
    \node (vk) [label=below:${u_2}$] at (-0.25cm, 0.0cm) {};
    ...
    \node (uj) [label=above right:${v_j}$] at (1.5cm, 0.5cm) {};
...
\begin{tikzpicture}[scale=1.6]
    ...
    \node (D) [scale=1.25] [draw=none, fill=none] at (0.65cm, 0.95cm) {$D$};
...
\begin{tikzpicture}[scale=1.6]
    ...
    \end{pgfonlayer}
\end{tikzpicture}
\caption{Algorithm~\ref{A:poh_linear}, Step~3: Case~3.2 (left), Case~3.3 (middle),
...
\begin{tikzpicture}[scale=1.6]
    \node (v1) [label=above:${u_1}$] at (0.0cm, 1.0cm) {};
    \node (vi) [label=above:${w_1}$] at (2.75cm, 1.0cm) {};
    \node (vi1) [label=below:${w_2}$] at (2.75cm, 0.0cm) {};
    \node (vs) [label=below:${s}$] at (0.75cm, -0.25cm) {};
    \node (vt) [label=above:${v}$] at (1.25cm, 1.3cm) {};
    \node (vl) [label=above:${t}$] at (2.15cm, 1.25cm) {};
    \node (vk) [label=below:${u_2}$] at (0.0cm, 0.0cm) {};
    \node (u1) [label=below right:${v_1}$] at (0.25cm, 0.5cm) {};
    \node (ur) [label=above:${v_i}$] at (0.75cm, 0.5cm) {};
    \node (uj1) [label=below:${v_{j-1}}$] at (1.25cm, 0.5cm) {};
    \node (uj) [label=below:${v_j}$] at (1.75cm, 0.5cm) {};
    \node (y) [label=below:${y}$] at (2.25cm, 0.5cm) {};

    \begin{pgfonlayer}{bg}
        \draw (v1) edge[dashed, bend left=15] (vt);
        \draw (vt) edge[dashed, bend left=15] (vl);
...
\begin{tikzpicture}[scale=1.6]
    ...
    \node (vl) [label=above:${t}$] at (2.15cm, 1.15cm) {};
    \node (vk) [label=below:${u_2}$] at (0.0cm, 0.0cm) {};
    \node (u1) [label=below right:${v_1}$] at (0.25cm, 0.5cm) {};
    \node (ur) [label=above:${v_i}$] at (0.75cm, 0.5cm) {};
    ...
    \end{pgfonlayer}
\end{tikzpicture}
\caption{Algorithm~\ref{A:poh_linear}, Step~4: Case~4.2 when $v\in P_1$ (left)
and $v\in P_3$ (right).}\label{F:poh_linear_2}
\end{center}
\end{figure}
...
% For $v in N(u_j)$, define $\text{next}(v)$ to be the neighbor
% immediately counter-clockwise from $v$.
% We will iterate through the neighbors of $u_j$, proceeding
% counter-clockwise from immediately after $u_{j-1}$, until we
% encounter a vertex in
% $P_1$; such a neighbor is guaranteed since $u_j\in N(P_3)$.
% Let $v$ be the current neighbor. There are several cases to consider.

% \textbf{Step 3.}
% \textbf{Case 2.1} If $c[v]=0$ and $S[v]\ne m(P_1)$,
% assign $S[v]\rightarrow m(P_3)$ and a.
...
% $c[v]\leftarrow c(P_3)\in \{1,2,3\}-\{c(P_1),c(P_2)\}$.
% Each $v\in N(P_3)$ will be marked $S[v]\leftarrow m(P_3)$, where $m(P_3)$
...
% \textbf{Procedure.}
% We will construct $P_3$ iteratively, one vertex at a time.
% Let $u_1,u_2,\ldots,u_j$ be the current path, where $u_1=u$;
% let $u_0=v_k$ so that $u_{j-1}$ is defined for $j=1$.
% L
% Let $u_j$ be the last
% vertex added to
...
% $u_{j+1}$ once all other neighbors of $u_j$ have been handled. Store
...
% $v\in\text{Int}(C)-C-N(P_1)$. Assign $S[v]\leftarrow m(P_3)$.

% \textbf{Case 1.1.2.} Suppose that $c[v]=c(P_2)$, that is, $v=v_t\in P_2$.
...
% $N(P_3)\cap V(\text{Int}(C'))$ has already been marked with $m(P_3)$.
% Let
...
% \textbf{Case 1.1.3.} Suppose that $c[v]=0$ and $S[v]=m(P_1)$, that is,
% $v\in N(P_1)-C$. If $y\ne\text{NULL}$, assign $S[v]\leftarrow m(P_3)$.
% Otherwise, assign $y\leftarrow v$ and color
% $c[y]\leftarrow c(P_3)$. We claim that $u_1,u_2,\ldots,u_j,y$ is an induced
...
% \textbf{Case 1.1.4.} Suppose that $c[v]=c(P_1)$, that is, $v\in P_1$.
% Assign $u_jv_\ell\leftarrow u_jv$. If $y=\text{NULL}$, it must be
% that $v=v_i$ and $u_j=w$.
% To see this, note
% that the neighbor $v'$ of $u_j$ immediately clockwise from $v$ is adjacent
% to $v$, but $v'$ was not assigned to $y$, nor was $u_jv'$ assigned
% to $u_jv_\ell$. Therefore $c[v']=c(P_2)$, and it must be
% that $v'=v_{i+1}$ and $v=v_i$.
...
% Assign $S[v]\leftarrow m(P_3)$.

% \textbf{Case 1.2.2.} Suppose that $c[v]=c(P_1)$. Then
% $v=v_t\in P_1$ and $C'=u_j,v_t,v_{t+1},\ldots,v_\ell$ is an
% induced
% cycle. Each uncolored vertex in $N(P_3)\cap
% V(\text{Int}(C'))$ has been marked with $m(P_3)$.
...
% If $c[v]=c(P_2)$, then $j=1$, $v=v_k$, $u_jv_\ell=uv_1$, and there are no
% uncolored neighbors of $u_j$ between $v_\ell$ and $v$.

% Suppose that $c[v]=c(P_3)$. Let $t$ be largest such that
...
$v_{j}\not\in\text{Int}(D)$; see Case~3.3 in Figure~\ref{F:poh_linear_1}.
Therefore $v_kv_j\not\in\text{Int}(D)$ and
...
\v{S}krekovski's papers, but it ``localizes'' the inductive step to
consider one edge at a time. The resulting algorithm
...
$v\in G-C[x,y]$ satisfies $|L(v)|\ge 2$. Choose
...
\textbf{Case 3.1.}
Suppose that $u_2\in G-C$. Define $L'(u_2)=L(u_2)\setminus \{c(z)\}$ and
...
$n(C)>2$ and therefore $C$ is a cycle. Define $u_2=\text{next}_{z}(u_1)$.
...
$N[u_2]\leftarrow (s_3+1,s_2)$. To represent the subgraph $G_2$,
...
corresponding adjacency list arrays were re-ordered such that the list
for newly-labelled vertex $0$ came first in memory, followed
by the list for vertex $1$, and so on. The
benchmarks results therefore represent each algorithm's
performance on a set of uniformly sampled random plane triangulations with
uniformly sampled random representations in memory, independent of
our specific implementation of the
Poulalhon-Schaeffer algorithm.
...
The benchmark binaries and
source code are included with the implementation~\cite{Bro2017}.

The list assignments for Algorithm~\ref{A:hartman_impl} were uniformly
drawn from a set of six colors. Experiments drawing from larger color sets
showed slightly better performance for Algorithm~\ref{A:hartman_impl}, but also
resulted in boring colorings that induced mostly short paths.

The performance of
Algorithm~\ref{A:hartman_impl} was close to
Algorithm~\ref{A:poh_linear} on the random sample graphs.
However, Algorithm~\ref{A:hartman_impl} required an
augmented adjacency list representation to be computed with
...
vertex labels and adjacency list memory order were uniformly
randomized to remove dependence on one particular representation of the
pyramid graph. Values of $k$ were selected such that
$n(A_k)\approx 10^i$.

\begin{figure}[ht]
\begin{center}
\begin{tabular}{r||l|l|l|l}
    & $n\approx10^{4}$ & $n\approx10^{5}$ & $n\approx10^{6}$
        & $n\approx10^{7}$ \\
\hline
\hline
    Algorithm~\ref{A:poh_bfs} & 
    $9.67\cdot 10^{6}$ &
    $4.72\cdot 10^{8}$ & $7.38\cdot 10^{10}$ &
    $2.96\cdot 10^{12}$ \\
\hline
    Algorithm~\ref{A:poh_linear} & 
    $2.70\cdot 10^{5}$ &
    $3.82\cdot 10^{6}$ & $1.50\cdot 10^{8}$ &
    $2.28\cdot 10^{9}$ \\
\end{tabular}
...
\hline
    ...
    $4.65\cdot 10^{5}$ &
    $4.49\cdot 10^{6}$ & $9.11\cdot 10^{7}$ &
    $1.05\cdot 10^{9}$ \\
\hline
    3 & % $4.28\cdot 10^{2}$ & $6.48\cdot 10^3$ &
    $3.60\cdot 10^{5}$ &
    $3.50\cdot 10^{6}$ & $6.80\cdot 10^{7}$ &
    $7.88\cdot 10^{8}$ \\
\hline
    4 & % $4.28\cdot 10^{2}$ & $6.48\cdot 10^3$ &
    $3.14\cdot 10^{5}$ &
    $2.99\cdot 10^{6}$ & $5.64\cdot 10^{7}$ &
    $6.57\cdot 10^{8}$ \\
\end{tabular}
...
parallel Algorithm~\ref{A:poh_linear} to the same sample of plane
...
\hline
    ...
    $6.03\cdot 10^{5}$ &
    $7.02\cdot 10^{6}$ & $1.24\cdot 10^{8}$ &
    $1.40\cdot 10^{9}$ \\
\hline
    3 & % $7.62\cdot 10^{2}$ & $9.30\cdot 10^3$ &
    $5.03\cdot 10^{5}$ &
    $5.73\cdot 10^{6}$ & $9.70\cdot 10^{7}$ &
    $1.06\cdot 10^{9}$ \\
\hline
    4 & % $7.62\cdot 10^{2}$ & $9.30\cdot 10^3$ &
    $4.85\cdot 10^{5}$ &
    $5.69\cdot 10^{6}$ & $8.33\cdot 10^{7}$ &
    $8.96\cdot 10^{8}$ \\
\end{tabular}
...
showed some performance benefits when applying the parallel
version of Algorithm~\ref{A:hartman_impl} to plane graphs, with
significant benefits only showing for very large graphs.
