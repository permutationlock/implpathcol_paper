...
of $S$ and the edges in $G$ with both incident vertices in $S$.
...
plane embedded and triangulated in $\mathcal{O}(n)$ time, see Hopcroft
...
\begin{tikzpicture}
  ...
  \node at (0.0,-0.6) {$10^3$};
  \node at (1.25,-0.6) {$10^4$};
  \node at (2.5,-0.6) {$10^5$};
  \node at (3.75,-0.6) {$10^6$};
  \node at (5.0,-0.6) {$10^7$};
...
\begin{tikzpicture}
  ...
  \draw[very thick,color=red] (0, 0.216) -- (1.25, 0.987) -- (2.5, 1.907) -- (3.75, 3.128)
    -- (5, 4.016);
\end{tikzpicture}
\caption{Plot of Figure~\ref{F:benchmark} (left) and Figure~\ref{F:pyramid_benchmark} (right).}
    \label{F:plot}
\end{center}
\end{figure}
