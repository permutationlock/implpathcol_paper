...
the neighbor $u$. During Step 3 in Algorithm \ref{A:augment} suppose we reach
the vertex $v$. For every edge $uw\in E(G)$ such that
$u<w$ and $v<w$, we shall have initialized the references for $uw$ in Adj'[$u$]
and Adj'[$w$], as well as removed the pair $(w,r_{w,u})$ from Wrk[$u$]. The
subsequent iteration through Wrk[$v$] will handle all edges $uv$ with $v>u$.
...
cycle $C=v_1,v_2,\ldots, v_k$. Additionally, a $2$-coloring of $C$ such
that each color class induces a path, $P_1=v_1,v_2,\ldots, v_l$ and
$P_2=v_{l+1},v_{l+2},\ldots, v_k$ respectively.

\textbf{Output:} An extension of the path $2$-coloring of $C$ to a path
