...
$u,w$-path and/or locate a chord edge.
...
Such a search may be implemented by only searching $v\in G$
...
interior pyramid shaped induced subgraph with sides of length $r=k-i$, starting
from the vertex in the bottom left corner. The search will hit all
$r(r+1)/2$ vertices of the interior pyramid.
Therefore the number of operations performed will be
\[
...
is a new unique integer mark. For each induced cycle
$C'$ formed by colored vertices, we will make a recursive call to
path $3$-color $\text{Int}(C')-C'$.
...
A C implementation of each algorithm is available~\cite{BroFIFO 2017}.
...
Algorithm~\ref{A:hartman_impl} relied on a strict sequential
ordering. If we store the mark
and neighbor range ($S[v]$ and $N[v]$) for $x$, $y$, and $z$ alongside
each frame, then the recursive calls become order independent in
...
to a frame that will become ready when $v$ has been colored, i.e. had
its color list reduced to length $1$. To allow
...
the array $M$.
Uniqueness accross threads can be ensured by using a shared atomic mark
counter.
